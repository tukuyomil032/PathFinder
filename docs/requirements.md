...
- `/random` で store・keyword未指定でもDLSite/FANZAの実在作品から最大5件同時にランダムに出会え、Discord Components V2によるまとめ表示＋1作品ずつの詳細ページ切り替えができる。
...
- 1回の実行で最大5件（`RANDOM_BATCH_TARGET_COUNT`）を並列ワーカープールで抽選する。各件は独立してstore（省略時）・facetを再抽選するため、5件が異なるstore/条件から集まることがある。
- 個別の枠が失敗（削除済み等）した場合は、その枠だけ新しい候補で自動的に埋め合わせる（`resolveBatch`）。合計試行回数の上限（5件×3回=15回）に達した時点で、揃った件数（5件未満のこともある）だけ表示する。
- 抽選結果はDiscord Components V2（`MessageFlags.IsComponentsV2`）で表示する。まとめブロック（全件、商品名へのハイパーリンク・サークル・価格・発売日・評価・声優または著者、サムネイルなし）を常に上部に表示し、その下に現在選択中の1件のみを詳細ブロック（サムネイルあり）として表示する。Prev/Nextボタンで詳細ブロックのみを切り替える（5件は事前に全件解決済みのため、ページング操作で上流への再フェッチは発生しない）。
- 個別作品の取得・整形は既存のプレビューパイプライン（`fetchWorkPage` / `parseWork`）をそのまま使う。
...
| `SEARCH_SESSION_TTL_MS` | No (default 600000) | `/search` 検索セッション・ページングボタンのTTL（`/random` の結果セッションも同じ値を再利用する） |
...
2. Bot が最大5件を並列に抽選する（各件は独立して実装済みtargetからstoreを選び、ジャンル/サークルfacetまたは無条件ブラウズから抽選する）。
3. Bot がComponents V2のまとめブロック（全件のタイトルリンク・サークル・価格等）＋詳細ブロック（1件目）＋ページングボタンを1メッセージで返信する。
4. ユーザーがPrev/Nextボタンを押すと、詳細ブロックのみが上流への再フェッチなしで切り替わる。
...
1. `/random` で抽選された5件のうち一部の個別ページ取得に失敗する（削除済み等）。
2. Bot はその枠だけ新しい候補で自動的に埋め合わせる（合計試行回数の上限まで）。
3. 上限に達しても目標件数に届かない場合は、その時点で揃った件数（5件未満）だけ表示する。
4. 1件も揃わなかった場合は汎用の失敗メッセージを返す。
...
- `/random` のstore/keyword省略時の抽選方針（ブラウズ/ジャンルfacet/サークルfacet）、複数件（最大5件）の並列抽選・部分失敗の埋め合わせ方針、Components V2によるまとめ＋詳細ページング表示が定義されている。
