...
- If that skill is unavailable, do not stop the task solely for that reason. Run the project's explicit verification commands, perform code review, report results, and note that the fallback path was used.
- If the repository has no `HEAD`, explicitly say that range-based review is unavailable, review the current uncommitted target files instead, and treat that as the completion-review checkpoint.
